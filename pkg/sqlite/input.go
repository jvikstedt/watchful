...
func (s *sqlite) InputCreate(input *model.Input) error {
	result, err := s.q.Exec(`INSERT INTO inputs (task_id, name, value, created_at, updated_at) VALUES (?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`, input.TaskID, input.Name, input.Value)
	if err != nil {
		return err
	}
...
func (s *sqlite) InputUpdate(input *model.Input) error {
	_, err := s.q.Exec(`UPDATE inputs SET value = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`, input.Value, input.ID)
	if err != nil {
		return err
	}
